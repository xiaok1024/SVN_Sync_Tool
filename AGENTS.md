...
- 终端版（`svn_sync_cli.py`）：macOS 主要使用方式，交互菜单 + 子命令参数两种用法；`CliEngine` 继承无界面的 `svn_sync_core.SyncEngine`，并复用 `svn_sync_tool.py` 中的升级清单解析 API。

当前主要技术栈：

- Python：`svn_sync_tool.py`（GUI + 升级清单解析）、`svn_sync_cli.py`（终端入口）、`svn_sync_core.py`（共享核心）、`svn_path_generator.py`（Tab 5 / paths）、`svn_standard_file_core.py` / `svn_standard_file_tab.py`（Tab 6 / standard）
...
- `SyncEngine` 负责 GUI/CLI 共享的 SVN 命令、SMB/UNC 解析、锁处理和文件扫描；新增无界面能力优先放入共享核心。
- `StandardFileService` 负责标准文件扫描、路径安全、覆盖和提交准备；`SvnStandardFileTab` 只负责 Tab 6 界面与线程调度。
